...
using namespace mathx::interpolation;
#include <string>

bspline::bspline(const std::vector<Eigen::VectorXd>& control_points, const size_t degree) :n_(control_points.size()), p_(degree), control_points_(control_points){
...
bspline::bspline(const bspline &other) : n_(other.n_),  p_(other.p_), control_points_(other.control_points_),knots_(other.knots_) {
}
...
bspline::bspline(const std::vector<Eigen::VectorXd>& control_points, size_t degree, const std::vector<double>& knots)
    ...
    if (knots_.front() != 0.0 || knots_.back() != 1.0) throw std::invalid_argument("knot vector must be clamped to [0,1]");
}
...
static std::vector<double> parameterize(const std::vector<Eigen::VectorXd>& pts, const std::string& method) {
    ...
    double acc = 0.0;
    for (size_t i=1;i<m-1;++i) { acc += (pts[i]-pts[i-1]).norm(); u[i] = acc/total; }
    u.back() = 1.0; u.front() = 0.0;
    return u;
}
...
static std::vector<double> interpolation_knots(const std::vector<double>& u, size_t degree) {
    ...
    if (m > p+1) {
        for (size_t j=1; j < m - p; ++j) { // j = 1 .. m-p-1 inclusive
            double s = 0.0;
...
bspline bspline::interpolate(const std::vector<Eigen::VectorXd>& data_points,
                             size_t degree,
    ...
    std::vector<Eigen::VectorXd> dummyCPs; dummyCPs.reserve(m);
    ...
    return bspline(cps, degree, knots);
}

bspline bspline::smooth_interpolate(const std::vector<Eigen::VectorXd>& data_points,
                                    size_t degree,
                                    double lambda,
                                    const std::string& parameterization) {
    ...
    auto build_knots = [](size_t cpCount_, size_t deg){
        ...
        for (size_t i=0;i<=mlast;++i) {
            if (i <= deg) U[i] = 0.0; else if (i >= cpCount_) U[i] = 1.0; else U[i] = double(i - deg)/double(cpCount_ - deg);
        }
        return U;
    };
