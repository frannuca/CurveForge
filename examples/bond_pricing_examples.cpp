...
void example1_basicBondPricing() {
    ...
    for (double yield = 0.03; yield <= 0.07; yield += 0.01) {
        ...
        std::string type = (price > 100) ? "Premium" : (price < 100) ? "Discount" : "Par";
...
void example3_carryRoll() {
    ...
    double funding_rate = 0.03; // Example funding rate (3%)

    // Scenario 1: Yield unchanged
    CarryRollMetrics scenario1 = calculateCarryRoll(bond, 0.05, 0.05, 0.5, funding_rate);
    ...
    CarryRollMetrics scenario2 = calculateCarryRoll(bond, 0.05, 0.04, 0.5, funding_rate);
    ...
    CarryRollMetrics scenario3 = calculateCarryRoll(bond, 0.05, 0.06, 0.5, funding_rate);
