...
int main() {
    ...
    for (int i=0;i<N;++i) {
        ...
        Eigen::Vector2d pt; pt<<x,y; noisy.push_back(pt);
    }
    ...
    std::vector<double> yExact; yExact.reserve(M);
    for (int i=0;i<M;++i) {
    ...
    for (double lambda : lambdas) {
        ...
        std::vector<double> ySmooth; ySmooth.reserve(M);
        for (int i=0;i<M;++i) {
            double u = xs[i];
        ...
        if (rSmooth < rExact * 0.85) { // at least 15% reduction
            passed = true;
            break;
        }
    }
